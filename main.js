...
/** Display-only helper: turns a raw room slot into its flower name,
 * for wording shown to the user (e.g. "pink set the melody playing").
 * Purely presentational — doesn't touch slot/flower assignment logic. */
function slotToFlower(slot) {
  if (slot === "user1") return "pink";
  if (slot === "user2") return "lavender";
  return slot;
}
...
/**
 * Day/night switching (Phase 4.1). Purely time-based, purely decorative —
 * reads environmentState.js's clock-only getTimePeriod() and hands the
 * result to ui.js's setTimeOfDay(), which flips the body classes that
 * drive the night-art crossfade and firefly/butterfly layers in CSS. No
 * Firestore involved: each device switches independently off its own
 * clock, same as environmentState.js was designed to do.
 */
const ENVIRONMENT_POLL_MS = 60 * 1000;
...
async function joinRoom(preferredFlower) {
  ...
  renderConnectionStatus("Reaching out across the garden...");
  ...
  try {
    ...
    if (error.message === "SLOT_TAKEN") {
      renderConnectionStatus(`${preferredFlower === "pink" ? "Pink" : "Lavender"}'s bloom is already claimed.`);
      setGardenFlowerUnavailable(preferredFlower);
    } else if (error.message === "ROOM_FULL") {
      renderConnectionStatus("This garden only has room for two.");
    } else {
      console.error("Failed to join room:", error);
      renderConnectionStatus("Something rustled the connection — try again.");
    }
    joining = false;
 ...
 stopOtherFlowerListener = listenToFlowerById(otherFlowerId(), (layers) => {
  ...
  stopRoomListener = listenToRoom((roomData) => {
    ...
    if (!roomData || !roomData.user1Id || !roomData.user2Id) {
      renderConnectionStatus("Waiting for your companion to arrive...");
    } else {
  ...
  stopPlayerListener = listenToPlayer((playerData) => {
    ...
    renderLastAction(playerData.lastAction, slotToFlower(playerData.actionBy));
